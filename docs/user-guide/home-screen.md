...
`_` or `.` — where every engine puts its own files, and where a repository puts
`.git`. A `key=value` name is a partition whatever it begins with, so a dataset
partitioned on `_date` is not skipped. The first four skipped are named.

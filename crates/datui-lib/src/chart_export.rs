...
use crate::chart_data::{format_axis_label, format_x_axis_label, BoxPlotData, HeatmapData, XAxisTemporalKind};
...
pub fn write_heatmap_eps(path: &Path, data: &HeatmapData, bounds: &ChartExportBounds) -> Result<()> {
    if data.counts.is_empty() || data.max_count <= 0.0 {
        return Err(color_eyre::eyre::eyre!("No data to export"));
    }

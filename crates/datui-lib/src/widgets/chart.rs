...
pub fn render_chart_view(
    ...
    match modal.chart_kind {
        ChartKind::XY => {
            ...
            for (idx, (t, label)) in type_options.iter().enumerate() {
                ...
                Paragraph::new(Line::from(Span::styled(cell, style)))
                    .render(type_grid[idx], buf);
            }
        ...
        ChartKind::Heatmap => {
            ...
            let x_selected_set: HashSet<String> =
                modal.heatmap_x_column.iter().cloned().collect();
            let y_selected_set: HashSet<String> =
                modal.heatmap_y_column.iter().cloned().collect();
            let sidebar_content = Layout::default()
                .direction(Direction::Vertical)
                .constraints([
    ...
    match render_data {
        ...
        ChartRenderData::Histogram { data } => render_histogram_chart(
            chart_inner,
            buf,
            theme,
            data,
...
fn render_histogram_chart(
    ...
    Chart::new(vec![dataset]).x_axis(x_axis).y_axis(y_axis).render(area, buf);
}
...
fn render_kde_chart(
    ...
    let x_axis = Axis::default()
        ...
        .style(Style::default().fg(theme.get("text_primary")))
        .labels(vec![
            Span::styled(format_axis_label(data.x_min), Style::default().fg(theme.get("text_primary"))),
            Span::styled(
                format_axis_label((data.x_min + data.x_max) / 2.0),
                Style::default().fg(theme.get("text_primary")),
            ),
            Span::styled(format_axis_label(data.x_max), Style::default().fg(theme.get("text_primary"))),
        ]);
    let y_axis = Axis::default()
        .title("Density")
        ...
        .style(Style::default().fg(theme.get("text_primary")))
        .labels(vec![
            Span::styled(format_axis_label(0.0), Style::default().fg(theme.get("text_primary"))),
            Span::styled(
                format_axis_label(data.y_max / 2.0),
                Style::default().fg(theme.get("text_primary")),
            ),
            Span::styled(format_axis_label(data.y_max), Style::default().fg(theme.get("text_primary"))),
        ]);
...
fn render_box_plot_chart(
    ...
    for (i, stat) in data.stats.iter().enumerate() {
        ...
        segments.push(vec![(x - box_half, stat.median), (x + box_half, stat.median)]);
        segment_styles.push(style);
    ...
    Chart::new(datasets).x_axis(x_axis).y_axis(y_axis).render(area, buf);
}
...
fn render_heatmap_chart(
    ...
    let body = Layout::default()
        .direction(Direction::Horizontal)
        .constraints([
            Constraint::Length(y_label_width + 1),
            Constraint::Min(1),
        ])
        .split(layout[1]);
    ...
    for row in 0..plot_area.height {
        for col in 0..plot_area.width {
            ...
            let y_bin_raw =
                ((row as f64 / plot_area.height as f64) * data.y_bins as f64).floor();
            let y_bin = data
                .y_bins
                .saturating_sub(1)
            ...
            let cell = buf.get_mut(plot_area.x + col, plot_area.y + row);

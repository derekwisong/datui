    ...
    fn key(&mut self, event: &KeyEvent) -> Option<AppEvent> {
        ...
        if self.input_mode == InputMode::Chart {
            ...
            match event.code {
                ...
                KeyCode::Enter | KeyCode::Char(' ') if event.is_press() => match self.chart_modal.focus {
            ...
            KeyCode::Char('c') => {
                if let Some(state) = &self.data_table_state {
                    if self.input_mode == InputMode::Normal {
                        ...
                        self.chart_modal.heatmap_x_input = std::mem::take(
                            &mut self.chart_modal.heatmap_x_input,
                        )
                        .with_theme(&self.theme);
                        self.chart_modal.heatmap_y_input = std::mem::take(
                            &mut self.chart_modal.heatmap_y_input,
                        )
                        .with_theme(&self.theme);
    ...
    fn do_chart_export(
        ...
        match self.chart_modal.chart_kind {
            ...
            ChartKind::Histogram => {
                ...
                let data =
                    chart_data::prepare_histogram_data(&state.lf, &column, self.chart_modal.hist_bins)?;
                ...
                match format {
                    ChartExportFormat::Png => write_chart_png(path, &series, ChartType::Bar, &bounds),
                    ChartExportFormat::Eps => write_chart_eps(path, &series, ChartType::Bar, &bounds),
                }
            }
            ChartKind::BoxPlot => {
                let column = self
                ...
                let data = chart_data::prepare_box_plot_data(&state.lf, &[column.clone()])?;
            ...
            ChartKind::Kde => {
                ...
                match format {
                    ChartExportFormat::Png => write_chart_png(path, &series, ChartType::Line, &bounds),
                    ChartExportFormat::Eps => write_chart_eps(path, &series, ChartType::Line, &bounds),
                }
            }
            ChartKind::Heatmap => {
                let x_column = self
            ...
            match self.chart_modal.chart_kind {
                ...
                ChartKind::BoxPlot => {
                    if let (Some(state), Some(column)) = (
                        ...
                        box_data =
                            chart_data::prepare_box_plot_data(&state.lf, &[column]).ok();
                    }
                }
                ChartKind::Kde => {
                    if let (Some(state), Some(column)) = (
